...
Let’s go ahead and read in the network data, saved as an
edgelist.
...
Let’s also read in the attribute
file.
...
```
    ...
    ## IGRAPH 83af383 DNW- 24 77 -- 
    ## + attr: name (v/c), gender (v/c), grade (v/n), race (v/c), weight (e/n)
    ## + edges from 83af383 (vertex names):
...
the labels to make them a little easier to
read.
...
black edges around the nodes using a vertex.frame
argument.
...
the edges for a nice effect (setting edge.curved to
.15).
...
may want more control than that. For example, using an MDS-based
layout:
...
Now, let’s do the same plot but add arrows to it, capturing
direction.
...
frame. Let’s start with the edge spells data
frame.
...
And now we read in the vertex spells data
frame:
...
moving the interval up by 1 in each
layout.
